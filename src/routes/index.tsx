...
function Index() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-brand-primary">
      ...
      <section id="industries" className="px-6 py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Industry Experience" note="81+ businesses supported across 17 industries." /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{industryNames.map((industry, index) => <div key={industry} className="flex min-h-32 flex-col justify-between rounded-lg border border-slate-100 p-6 shadow-sm"><span className="text-sm font-bold text-brand-accent">{String(index + 1).padStart(2, "0")}</span><h3 className="mt-5 font-bold">{industry}</h3></div>)}</div></div></section>

      <section className="bg-surface-light px-6 py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="AI & Automation for Business Growth" note="Practical AI and automation to reduce manual work, improve workflows and support better decisions — not AI software engineering." /><div className="grid gap-12 lg:grid-cols-3"><div className="lg:col-span-2"><h3 className="mb-6 text-lg font-bold">Capabilities</h3><div className="grid gap-px overflow-hidden rounded-xl bg-slate-200 sm:grid-cols-2 lg:grid-cols-3">{aiCapabilities.map((capability) => <div key={capability} className="bg-white p-5 font-medium">{capability}</div>)}</div></div><div><h3 className="mb-6 text-lg font-bold">Tools</h3><div className="flex flex-wrap gap-3">{aiTools.map((tool) => <span key={tool} className="rounded-md bg-white px-4 py-2 font-semibold ring-1 ring-slate-200">{tool}</span>)}</div></div></div></div></section>

      <section id="experience" className="px-6 py-24"><div className="mx-auto max-w-5xl"><SectionHeading title="Experience & Qualifications" note="Career history, recognition and professional development." /><div className="relative space-y-10 before:absolute before:left-[15px] before:h-full before:w-px before:bg-slate-200">{experience.map((role) => <article key={role.role + role.company} className="relative pl-12"><div className="absolute left-0 top-1 h-8 w-8 rounded-full border-4 border-white bg-brand-accent" /><p className="font-bold text-brand-accent">{role.period}</p><h3 className="mt-1 text-xl font-bold">{role.role}</h3><p className="text-brand-muted">{role.company} · {role.place}</p><p className="mt-4 leading-relaxed text-brand-muted">{role.outcome}</p></article>)}</div><div className="mt-20 grid gap-10 border-t border-slate-200 pt-12 md:grid-cols-2"><div><h3 className="mb-5 text-xl font-bold">Awards & Recognition</h3><ul className="space-y-4">{awards.map((item) => <li key={item.title}><p className="font-semibold">{item.title}</p><p className="text-sm text-brand-muted">{item.org}</p></li>)}</ul></div><div><h3 className="mb-5 text-xl font-bold">Education & Certifications</h3><ul className="space-y-4">{certifications.map((item) => <li key={item.year + item.title} className="flex gap-4"><span className="w-12 shrink-0 text-sm font-bold text-brand-accent">{item.year}</span><span><span className="block font-semibold">{item.title}</span><span className="text-sm text-brand-muted">{item.issuer}</span></span></li>)}</ul></div></div></div></section>
